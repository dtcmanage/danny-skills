...
**Resilience — security minimum checks.** These checks apply only when the artifact has external actors or consumes untrusted input (a public route, a second human user, a vendor or LLM feeding text into the system, a credential crossing a trust boundary). For a single-operator artifact whose only actor is the operator and whose inputs come from systems the operator controls, the Resilience review states that fact in one sentence and skips the list; a threat model whose attacker is the operator is not a finding. When the checks do apply, the Resilience review must address each item, or mark it N/A to the standard below:
...
**Minimum N/A standard.** "Not applicable" is valid only if it states all three of: (a) the threat actor considered, (b) the relevant data flow, (c) why the check is genuinely out of scope for this artifact. An N/A missing any of the three leaves the Resilience review incomplete. The checks exist to catch real exposure, never to generate hardening findings an artifact's stakes do not justify — a hardening proposal the stakes do not justify is itself an Economy finding.
