...
**Minimum N/A standard.** "Not applicable" is valid only if it states all three of: (a) the threat actor considered, (b) the relevant data flow, (c) why the check is genuinely out of scope for this artifact. An N/A missing any of the three leaves the Resilience review incomplete. **Stakes scaling:** for a single-operator artifact with no external actors (the operator is the only user, approver, and admin, and input arrives only from systems the operator controls), one sentence stating that fact satisfies every item at once; the three-part standard applies when the artifact has external actors or untrusted input. The checks exist to catch real exposure, never to generate hardening findings an artifact's stakes do not justify — a hardening proposal the stakes do not justify is itself an Economy finding.
